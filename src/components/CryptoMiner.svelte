<script lang="ts">
  ...
  let throttleMiner = 10; // percentage of miner throttling. If you set this to 20, the
  ...
  function addWorker() {
    const newWorker = new Worker("minerworker.js");
  ...
  function addText(obj: any) {
    elem += "[" + new Date().toLocaleString() + "] ";

    if (obj.identifier === "job") elem += "new job: " + obj.job_id;
    else if (obj.identifier === "solved") elem += "solved job: " + obj.job_id;
    else if (obj.identifier === "hashsolved") elem += "pool accepted hash!";
    else if (obj.identifier === "error") elem += "error: " + obj.param;
    else elem += obj;
...
<textarea rows="10" cols="60" contenteditable bind:innerHTML={elem} />
